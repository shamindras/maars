...
get_summary2 <- function(x, sand=TRUE, boot_emp=FALSE, boot_res=FALSE,
                        boot_mul=FALSE,
...
get_assumptions <- function(x, sand=sand, boot_emp=boot_emp,
                            boot_res=boot_res,
                            boot_mul=boot_mul,
...
summary.maars_lm <- function(x, sand=TRUE, boot_emp=FALSE, boot_res=FALSE,
    ...
    out_summ <- get_summary2(x, sand=sand, boot_emp=boot_emp,
                             boot_res=boot_res,
                            boot_mul=boot_mul,
            well_specified=well_specified,...)
...
mss_var <- function(mod_fit, boot_emp = NULL, boot_res = NULL,
    ...
    if(all(c('maars_lm', 'lm') %in% class(mod_fit))) attr(mod_fit, 'class') <-
            'lm'
