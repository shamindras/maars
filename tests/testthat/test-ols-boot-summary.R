...
test_that("test estimate variance from empirical bootstrap matches estimate of variance from stats::lm", {
    ...
    boot_summary <- comp_bootstrap_summary(mod_fit = lm_fit, boot_out = boot_out, boot_type = 'emp')
...
test_that("test estimate variance from empirical bootstrap for different values of m match", {
    boot_out <- comp_empirical_bootstrap(lm_fit, B = 1e4, m = 50)
    boot_summary_50 <- comp_bootstrap_summary(mod_fit = lm_fit, boot_out = boot_out, boot_type = 'emp')
    boot_out <- comp_empirical_bootstrap(lm_fit, B = 1e4)
    boot_summary <- comp_bootstrap_summary(mod_fit = lm_fit, boot_out = boot_out, boot_type = 'emp')
...
test_that("test estimate variance from residual bootstrap matches estimate of variance from stats::lm", {
    ...
    boot_summary <- comp_bootstrap_summary(mod_fit = lm_fit, boot_out = boot_out, boot_type = 'res')
