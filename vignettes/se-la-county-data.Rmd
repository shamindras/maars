...
``` {r table1_img, echo=FALSE, fig.cap="Table 1 from [@buja2019modelsasapproximationspart1]", out.width = '90%', eval=FALSE}
...
``` {r emp_fit_lm, message=FALSE, warning=FALSE, eval=FALSE}
coef_emp_boot <- maars::comp_empirical_bootstrap(mod_fit = mod_fit, B = 1000)
...
``` {r emp_qqnorm, message=FALSE, warning=FALSE, eval=FALSE}
qnorm_boot <- maars::qqnorm_bootstrap(coef_emp_boot)
qnorm_boot
```

```{r emp_qqnorm_save_image, include = FALSE, eval=FALSE}
qnorm_boot <- maars::qqnorm_bootstrap(coef_emp_boot %>% tidyr::unnest(boot_out) %>% 
...
``` {r se_emp_fit_lm, message=FALSE, warning=FALSE, eval=FALSE}
se_emp <- maars::comp_bootstrap_summary(mod_fit = mod_fit, 
...
``` {r se_emp_fit_lm_table, eval=FALSE}
se_emp %>% 
  purrr::set_names(x = ., 
                   nm = c('Term', '$\\widehat{\\beta}_{j}$',
...
```{r se_emp_save_table, include=FALSE, eval=FALSE}
se_emp %>%
  purrr::set_names(x = ., 
                   nm = c('Term', '$\\widehat{\\beta}_{j}$',
                          '$SE_{\\text{boot}}$', '$t_{\\text{boot}}$',
...
```{r qqnorm, eval=FALSE}
coef_emp_boot %>% 
  maars::comp_conf_int_bootstrap(probs = c(0.025, 0.975)) %>%
...
Let's first get the OLS estimates. 
``` {r mult_fit_lm, message=FALSE, warning=FALSE, eval=FALSE}
coef_mult_boot <- maars::comp_multiplier_bootstrap_var(mod_fit = mod_fit, 
                                                      B = 1000, 
                                                      weights_type = "std_gaussian")
dplyr::glimpse(coef_mult_boot)
```

``` {r se_mult_fit_lm, message=FALSE, warning=FALSE, eval=FALSE}
# se_mult <- maars::comp_bootstrap_summary(mod_fit = mod_fit, 
...
``` {r se_sand_fit_lm, eval=FALSE}
se_sand <- maars::comp_sandwich_qr_var(mod_fit = mod_fit)
se_sand
```

```{r se_sand_save_table, include=FALSE, eval=FALSE}
se_sand %>%
...
```{r plot_std_sand_save_image, include = FALSE, eval=FALSE}
comp_se +
  ggplot2::theme(
...
```{r se_comb_save_table, include=FALSE, eval=FALSE}
se_comb %>%
  dplyr::select(-statistic, -p.value) %>% 
  purrr::set_names(x = ., 
                   nm = c('Term', '$\\widehat{\\beta}_{j}$', 
...
fs <- maars::focal_slope(mod_fit, coef_rwgt, 'PercVacant')
fs
```
