...
install.packages(pkgs = c("tidyverse", "vroom", "remotes", "glue", 
...
``` {r load_data, linewidth=80, message=FALSE}
...
se_comb %>%
  purrr::set_names(x = ., 
                   nm = c('Term', 'Estimate', '$SE_{\\text{lin}}$',
                          '$SE_{\\text{boot}}$', '$SE_{\\text{sand}}$')) %>% 
  knitr::kable(x = ., format = "html", digits = 3, align = "c", escape = TRUE)
```
